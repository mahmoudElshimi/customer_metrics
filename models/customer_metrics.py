...
class ResPartnerCustomerMetrics(models.Model):
    ...
    def _auto_create_customer_metrics(self):
        ...
        _logger.info(
            f"Starting Customer Metrics Initialization: {total_partners} customers found."
        )

        for offset in range(0, total_partners, BATCH_SIZE):
            partners = partner_model.search([], offset=offset, limit=BATCH_SIZE)
            existing_customers = customer_metrics_model.search(
                [("customer_id", "in", partners.ids)]
            )
            existing_ids = set(existing_customers.mapped("customer_id.id"))
            new_partners = [
                partner.id for partner in partners if partner.id not in existing_ids
            ]

            if new_partners:
                customer_metrics_model.create(
                    [{"customer_id": pid} for pid in new_partners]
                )
                self.env.cr.commit()  # Commit after processing each batch

            _logger.info(
                f"Processed {offset + len(partners)}/{total_partners} customers..."
            )
    ...
    def init(self):
        ...
        cron = self.env.ref(
            "customer_metrics.ir_cron_customer_metrics_init", raise_if_not_found=False
        )
        if cron:
